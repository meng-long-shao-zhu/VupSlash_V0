...
void CardOverview::addCard(int i, const Card *card)
{
    ...
    bool is_special_card = Sanguosha->translate(":"+card->objectName()).startsWith("<font color='#008080'>特殊牌</font>"); //识别特殊牌
    ...
    if (is_special_card) {
        package_item->setBackgroundColor(QColor(0xDD, 0xA0, 0xDD));
        package_item->setToolTip(Sanguosha->translate("SPECIAL_CARD_STATEMENT"));
    }else if (Config.value("LuaPackages", QString()).toString().split("+").contains(card->getPackage())) {
