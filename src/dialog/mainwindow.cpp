    ...
    : QMainWindow(parent), ui(new Ui::MainWindow), server(NULL)
{
    ...
    setWindowTitle(Sanguosha->translate("GAME_NAME") + " " + Sanguosha->getVersionNumber());
    ...
    connect(ui->actionView_winrate, SIGNAL(triggered()), this, SLOT(viewWinrate()));
...
void MainWindow::checkVersion(const QString &server_version, const QString &server_mod)
{
    ...
    text.append(Sanguosha->translate("ASK_FOR_UPDATE_LOG1"));
    text.append(Sanguosha->translate("ASK_FOR_UPDATE_LOG2"));
...
void MainWindow::enterRoom()
{
    ...
    ui->actionView_knowncard->setEnabled(true);
    //ui->actionView_winrate->setEnabled(true);
    ...
    connect(ui->actionView_knowncard, SIGNAL(triggered()), room_scene, SLOT(viewKnowncard()));
    //connect(ui->actionView_winrate, SIGNAL(triggered()), room_scene, SLOT(viewWinrate()));
...
void MainWindow::on_actionAbout_triggered()
{
    ...
    QString forum_url = "http://vupslash.icu/";
    ...
    QString content = Sanguosha->translate("ABOUT_VUPSLASH");

    Window *window = new Window(Sanguosha->translate("ABOUT_VUPSLASH_TITLE"), QSize(420, 470));
...
void MainWindow::on_actionRecord_analysis_triggered()
{
    ...
    if (labels.isEmpty()) {
        labels << tr("ScreenName") << Sanguosha->translate("CHARACTER") << tr("Role") << tr("Living") << tr("WinOrLose") << tr("TurnCount")
...
void MainWindow::viewWinrate()
{
    WinrateViewDialog *dialog = new WinrateViewDialog(this);
    dialog->show();
}
