    ...
    : QDialog(parent)
{
    ...
    foreach (const General *general, generals) {
        ...
        button->setToolTip(general->getSkillDescription(true, true));
        buttons << button;
    ...
    if (generals.length() <= columns) {
        ...
        if (lord_name.size() && !ServerInfo.EnableHegemony && !no_icon) {
            ...
            label->setToolTip(lord->getSkillDescription(true, true));
    ...
    if (!view_only && !ServerInfo.EnableHegemony) {
        ...
        if (lord_name.size()) role_label->setText(tr("The lord has chosen %1. Your seat is %2. %3")
            ...
            role_label->setText(tr("%1 %2%3%4").arg(Sanguosha->translate("SEAT_NUM_LOG1")).arg(Sanguosha->translate("CAPITAL(" + QString::number(Self->getSeat()) + ")"))
                                                            .arg(Sanguosha->translate("SEAT_NUM_LOG2")).arg(role_label->text()));
...
QWidget *FreeChooseDialog::createTab(const QList<const General *> &generals)
{
    ...
    for (int i = 0; i < generals.length(); i++) {
        ...
        if (general->isBonus())
            button->setIcon(bonus_star_icon);
