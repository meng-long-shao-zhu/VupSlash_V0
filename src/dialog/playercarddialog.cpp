    ...
    : player(player), handcard_visible(handcard_visible), method(method), disabled_ids(disabled_ids)
{
    ...
    QGroupBox *harea = new QGroupBox(tr("Handcard area"));
    QVBoxLayout *vlayout3 = new QVBoxLayout;
    harea->setLayout(vlayout3);
    if (flags.contains(handcard_flag)) {
        if (player->isKongcheng()) {
            vlayout3->addWidget(createHandcardButton());
        } else if (Self == player || handcard_visible) {    //all known
            vlayout3->addWidget(createHandcardArea());
        } else {
            if (player->getPublicCards().length() > 0)    //it means player has 0 overt_card
                vlayout3->addWidget(createHandcardArea());
            vlayout3->addWidget(createHandcardButton());
        }
        vlayout2->addWidget(harea);
    }
...
QWidget *PlayerCardDialog::createHandcardButton()
{
    ...
    if (num > 0)    //sometimes there will be residual information (information of last time), so it might be negative if minus it directly
        num -= player->getPublicCards().length();
    if (num == 0) {
        button->setDescription(Sanguosha->translate("NO_UNKNOWN_HANDCARD"));
        button->setEnabled(false);
    } else {
        button->setDescription(Sanguosha->translate("CHOOSE_UNKNOWN_HANDCARD1") + QString::number(num) + Sanguosha->translate("CHOOSE_UNKNOWN_HANDCARD2"));
...
QWidget *PlayerCardDialog::createHandcardArea()
{
    QGroupBox *area = new QGroupBox(Sanguosha->translate("OPTIONAL_HANDCARDS"));
    QVBoxLayout *layout = new QVBoxLayout;
    QList<const Card *> cards = (handcard_visible) ? player->getHandcards() : player->getPublicCards();
