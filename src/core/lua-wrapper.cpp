...
LuaTriggerSkill::LuaTriggerSkill(const char *name, Frequency frequency, const char *limit_mark, bool change_skill, bool limited_skill, bool hide_skill)
...
LuaOffensiveHorse *LuaOffensiveHorse::clone(Card::Suit suit, int number) const
{
    if (suit == Card::SuitToBeDecided) suit = this->getSuit();
    if (number == -1) number = this->getNumber();
    LuaOffensiveHorse *new_card = new LuaOffensiveHorse(suit, number, this->getCorrect(), objectName().toStdString().c_str(), class_name.toStdString().c_str());
...
LuaDefensiveHorse *LuaDefensiveHorse::clone(Card::Suit suit, int number) const
{
    if (suit == Card::SuitToBeDecided) suit = this->getSuit();
    if (number == -1) number = this->getNumber();
    LuaDefensiveHorse *new_card = new LuaDefensiveHorse(suit, number, this->getCorrect(), objectName().toStdString().c_str(), class_name.toStdString().c_str());
