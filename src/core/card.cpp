...
bool Card::CompareByType(const Card *a, const Card *b)
{
    ...
    if (order1 != order2)
        ...
        if (basic.isEmpty())
            basic << "slash" << "thunder_slash" << "fire_slash" << "ice_slash" << "dark_slash" << "jink" << "peach" << "analeptic";
        switch (a->getTypeId()) {
        case TypeBasic: {
...
void Card::onUse(Room *room, const CardUseStruct &use) const
{
    ...
    card_use = data.value<CardUseStruct>();
    thread->trigger(BeforeCardFinished, room, card_use.from, data);
...
bool Card::isOvert() const
{
    return is_overt;
}

void Card::setOvert(bool can)
{
    is_overt = can;
}

bool Card::noIndicator() const
{
    return no_indicator;
}
