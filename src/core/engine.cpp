...
void Engine::addPackage(Package *package)
{
    ...
    foreach (Card *card, all_cards) {
        ...
        } else if (card->isKindOf("LuaOffensiveHorse")) {
            const LuaOffensiveHorse *lcard = qobject_cast<const LuaOffensiveHorse *>(card);
            Q_ASSERT(lcard != NULL);
            luaOffensiveHorse_className2objectName.insert(lcard->getClassName(), lcard->objectName());
            if (!luaOffensiveHorses.keys().contains(lcard->getClassName()))
                luaOffensiveHorses.insert(lcard->getClassName(), lcard->clone());
        } else if (card->isKindOf("LuaDefensiveHorse")) {
            const LuaDefensiveHorse *lcard = qobject_cast<const LuaDefensiveHorse *>(card);
            Q_ASSERT(lcard != NULL);
            luaDefensiveHorse_className2objectName.insert(lcard->getClassName(), lcard->objectName());
            if (!luaDefensiveHorses.keys().contains(lcard->getClassName()))
                luaDefensiveHorses.insert(lcard->getClassName(), lcard->clone());
...
Card *Engine::cloneCard(const QString &name, Card::Suit suit, int number, const QStringList &flags) const
{
    ...
    if (luaBasicCard_className2objectName.keys().contains(name)) {
        ...
        if (!lcard) return NULL;
        card = lcard->clone(suit, number);
    } else if (luaOffensiveHorse_className2objectName.keys().contains(name)) {
        const LuaOffensiveHorse *lcard = luaOffensiveHorses.value(name, NULL);
        if (!lcard) return NULL;
        card = lcard->clone(suit, number);
    } else if (luaOffensiveHorse_className2objectName.values().contains(name)) {
        QString class_name = luaOffensiveHorse_className2objectName.key(name, name);
        const LuaOffensiveHorse *lcard = luaOffensiveHorses.value(class_name, NULL);
        if (!lcard) return NULL;
        card = lcard->clone(suit, number);
    } else if (luaDefensiveHorse_className2objectName.keys().contains(name)) {
        const LuaDefensiveHorse *lcard = luaDefensiveHorses.value(name, NULL);
        if (!lcard) return NULL;
        card = lcard->clone(suit, number);
    } else if (luaDefensiveHorse_className2objectName.values().contains(name)) {
        QString class_name = luaDefensiveHorse_className2objectName.key(name, name);
        const LuaDefensiveHorse *lcard = luaDefensiveHorses.value(class_name, NULL);
        if (!lcard) return NULL;
        card = lcard->clone(suit, number);
...
{
    //return "20210210";
    return Sanguosha->translate("CURRENT_VERSION");
}
#endif
...
QStringList Engine::getRandomLords(bool is_robot) const
{
    ...
    if (is_robot) {
        QFile file("etc/banned.txt");
        if (file.open(QIODevice::ReadOnly)) {
            QTextStream stream(&file);
            while (!stream.atEnd()) {
                QString name;
                stream >> name;
                banlist_ban.append(name);
            }
...
QStringList Engine::getAllGeneralNames(const QString &kingdom) const
{
    QStringList general_names;
    QHashIterator<QString, const General *> itor(generals);
    while (itor.hasNext()) {
...
QStringList Engine::getRandomGenerals(int count, const QSet<QString> &ban_set, const QString &kingdom, bool is_robot) const
{
    ...
    if (is_robot) {
        ...
        general_set = general_set.subtract(banned_list.toSet());
    }
