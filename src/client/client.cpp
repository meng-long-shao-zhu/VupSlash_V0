    ...
    player_count(1) // Self is not included!! Be care!!!
{
    ...
    m_callbacks[S_COMMAND_SET_PUBLIC_CARDS] = &Client::setPublicCards;
    m_callbacks[S_COMMAND_REFRESH_OVERT_PILE] = &Client::refreshOvertPile;
...
void Client::setPublicCards(const QVariant &set_str)
{
    JsonArray set = set_str.value<JsonArray>();
    if (set.size() != 2) return;
    QString name = set[0].toString();
...
{
    JsonArray args = arg.value<JsonArray>();
    if (args.size() != 2 || !JsonUtils::isBool(args[0]) || !JsonUtils::isString(args[1])) return;
    //if (!JsonUtils::isBool(arg)) return;
    m_isDiscardActionRefusable = args[0].toBool();
    m_askforAG_reason = args[1].toString();
    setStatus(AskForAG);
}
...
void Client::askForGuanxing(const QVariant &arg)
{
    ...
    int up_limit = args[1].toInt();
    int down_limit = args[2].toInt();
    QList<int> card_ids;
    JsonUtils::tryParse(deck, card_ids);

    emit guanxing(card_ids, up_limit, down_limit);
