...
void ClientPlayer::addCard(const Card *card, Place place)
{
    switch (place) {
    case PlaceHand: {
        if (card && !known_cards.contains(card)) known_cards << card;
        handcard_num++;
        break;
    }
    case PlaceEquip: {
...
/*int ClientPlayer::getOvertcardNum() const
{
    int count = 0;
    foreach (const Card *ncard, known_cards) {
        if (ncard->isOvert())
...
QList<const Card *> ClientPlayer::getPublicCards() const
{
    return public_cards;
}

void ClientPlayer::setPublicCards(const QList<int> &card_ids)
{
    public_cards.clear();
    foreach(int cardId, card_ids)
        public_cards.append(Sanguosha->getCard(cardId));
}
...
void ClientPlayer::refreshPile(const QString &name)
{
    if (!name.startsWith("#"))
        emit pile_changed(name);
}
