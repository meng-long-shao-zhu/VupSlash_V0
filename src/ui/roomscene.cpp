    ...
    : main_window(main_window), game_started(false), m_tableBgPixmap(1, 1), m_tableBgPixmapOrig(1, 1)
{
    ...
    connect(ClientInstance, SIGNAL(gongxin(QList<int>, bool, QList<int>, QString)), this, SLOT(doGongxin(QList<int>, bool, QList<int>, QString)));
    ...
    connect(ClientInstance, SIGNAL(guanxing(QList<int>, int, int)), guanxing_box, SLOT(doGuanxing(QList<int>, int, int)));
    ...
    chat_edit->setAttribute(Qt::WA_InputMethodEnabled, true);
    chat_edit->setAttribute(Qt::WA_KeyCompression, true);
    chat_edit->setFocusPolicy(Qt::WheelFocus);
...
void RoomScene::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (!item || item->zValue() < -99999) {
        ...
        if (ServerInfo.EnableCheat || canseehandcard) {*/
            ...
            QAction *action = known_cards->addAction(tr("View in new dialog"));
            //action->setData(player->objectName());
            connect(action, SIGNAL(triggered()), this, SLOT(viewKnowncard()));
            ...
            foreach (PlayerCardContainer *container, item2player.keys()) {
                ...
                if (known.isEmpty() || player->isKongcheng()) {
                    ...
                    foreach (const Card *card, known) {
                        ...
                        if (player->getPublicCards().contains(card))
                            submenu->addAction(G_ROOM_SKIN.getCardSuitPixmap(engine_card->getSuit()), engine_card->getFullName() + " " + Sanguosha->translate("OVERT"));
                        else
        ...
        //QAction *knowncard = menu->addAction(tr("View knowncard"));
        //connect(knowncard, SIGNAL(triggered()), this, SLOT(viewKnowncard()));
...
void RoomScene::chooseOption(const QString &skillName, const QStringList &options)
{
    ...
    QStringList sp_skillName = skillName.split("+");
    QString title_word = sp_skillName.at(0);
    sp_skillName.removeAt(0);
    QRegExp rx("sgs\\d+");
    if (sp_skillName.size() > 0) {
        QString log_text;
        foreach (QString logs, sp_skillName) {
            if (rx.exactMatch(logs)) {   //player_obj_name
                log_text += ClientInstance->getPlayerName(logs);
            } else {
                log_text += Sanguosha->translate(logs);
            }
        }
        QLabel *log_label = new QLabel(log_text);
        main_layout->addWidget(log_label);
    }

    QString title = Sanguosha->translate(title_word);
    dialog->setWindowTitle(title);

    QGroupBox *choices_group = new QGroupBox(Sanguosha->translate("CHOICES_GROUP"));
    choices_group->setLayout(layout);
    ...
    if (skillName.contains("guhuo") && !guhuo_log.isEmpty()) {
        ...
        main_layout->addWidget(guhuo_text);
    ...
    if (skillName.startsWith("BossModeExpStore")) {
        ...
        main_layout->addWidget(new QLabel(labelname));
    } else {
    ...
    if (skillName == "BossModeExpStore") {
        ...
        foreach (QString option, options) {
            QCommandLinkButton *button = new QCommandLinkButton;
            QString text = QString("%1:%2").arg(title_word).arg(option);
            QString translated = Sanguosha->translate(text);
            if (text == translated)
...
bool RoomScene::_shouldIgnoreDisplayMove(CardsMoveStruct &movement)
{
    ...
    if ((from == Player::PlaceSpecial && !from_pile.isEmpty() && from_pile.startsWith('#'))
        ...
        if (movement.reason.m_skillName != "manjuan" && ignore_place.contains(from) && ignore_place.contains(to)) return true;
        if (movement.reason.m_skillName == "hide_move_skill") return true;
    }
    return false;
}
...
void RoomScene::updateStatus(Client::Status oldStatus, Client::Status newStatus)
{
    ...
    case Client::AskForAG: {
        ...
        card_container->setLogButton(ClientInstance->m_askforAG_reason);
...
void RoomScene::fillTable(QTableWidget *table, const QList<const ClientPlayer *> &players)
{
    ...
    if (labels.isEmpty()) {
        labels << Sanguosha->translate("CHARACTER") << tr("Name") << tr("Alive");
        ...
        labels << tr("TurnCount");
        labels << tr("Recover") << tr("Damage") << tr("Damaged") << Sanguosha->translate("KillNum") /*<< tr("Designation")*/;
        labels << tr("Handcards");
...
void RoomScene::takeAmazingGrace(ClientPlayer *taker, int card_id, bool move_cards)
{
    ...
    if (taker) {
        ...
        if (move_cards) {
            ...
            move.to_place = Player::PlaceHand;
            move.to = taker;
            CardMoveReason reason(CardMoveReason::S_REASON_GOTBACK, taker->objectName());
            move.reason = reason;
...
void RoomScene::viewKnowncard()
{
    KnowncardViewDialog *dialog = new KnowncardViewDialog(main_window);
    dialog->show();
}
...
void RoomScene::doGongxin(const QList<int> &card_ids, bool enable_heart, QList<int> enabled_ids, const QString &player_name)
{
    QStringList foot_notes;
    for(int i=0;i<card_ids.length();i++)
        foot_notes.append(player_name + "+hand_area");
    QString foot_note_str = foot_notes.join("|");
    fillCards(card_ids, QList<int>(), false, foot_note_str);
    if (enable_heart)
...
void RoomScene::removeAnimate()
{
    QGraphicsObject *obj = qobject_cast<QGraphicsObject *>(sender());
    if (obj) {
        removeItem(obj->parentItem());
    }
}
...
void RoomScene::doLightboxAnimation(const QString &, const QStringList &args)
{
    ...
    if (!word.startsWith("skill=") && !word.startsWith("safe=")) {
    ...
    if (word.startsWith("image=")) {
...
#ifndef Q_OS_WINRT
    else if (word.startsWith("skill=") || word.startsWith("safe=")) {
        int len = 6;
        ...
        const QString hero = word.mid(len);
        ...
        connect(object, SIGNAL(animationCompleted()), this, SLOT(removeAnimate())); // remove lightbox
        addItem(object);
        bringToFront(lightbox);
        bringToFront(object);
    }
    /*else if (word.startsWith("play=")) {
        const QString hero = word.mid(5);
        const QString skill = args.value(1, QString());
        const QString file_name = "ui-script/"+word.split("=")[1].split(":")[0]+".qml";
        ...
        _m_animationComponent = new QDeclarativeComponent(_m_animationEngine, QUrl::fromLocalFile(file_name), this);
        QGraphicsObject *object = qobject_cast<QGraphicsObject *>(_m_animationComponent->create(_m_animationContext));
        connect(object, SIGNAL(animationCompleted()), object, SLOT(deleteLater())); // cannot replace?
        addItem(object);
        bringToFront(object);
...
void RoomScene::showIndicator(const QString &from, const QString &to)
{
    ...
    finish += QPointF(-20.0+qrand()%40, -20.0+qrand()%40);
...
void RoomScene::doPindianAnimation()
{
    ...
    if (pma) {
        QString to_emotion = pindian_other_success ? "success" : "no-success";
        PixmapAnimation::GetPixmapAnimation(pindian_to_card, to_emotion);
