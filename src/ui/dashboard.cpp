...
{
    if (m_player->getMark("Response_Time_Fix") > 0)
        countdown.max = m_player->getMark("Response_Time_Fix");
...
void Dashboard::addHandCards(QList<CardItem *> &card_items)
{
    foreach(CardItem *card_item, card_items) {
        QString text;
        QStringList flags = card_item->getCard()->getFlags();
        for (int i = 0; i < flags.length(); i++) {
            ...
            if (flag.startsWith("&")) {
                ...
                QStringList flag_names = flag.split("+");
                foreach (QString name, flag_names) {
                    text.append(Sanguosha->translate(name));
                }
                //break;
            }
        }
        _addHandCard(card_item, false, text);
    }
    updateHandcardNum();
}
    ...
    if (pending_card != new_pending_card) {
        ...
        if (view_as_skill->objectName().contains("guhuo")
            ...
            foreach (CardItem *item, m_handCards) {
                ...
                QString text;
                QStringList flags = item->getCard()->getFlags();
                for (int i = 0; i < flags.length(); i++) {
                    QString flag = flags.at(i);
                    if (flag.startsWith("&")) {
                        if (text != "") {
