...
void QSanSkillButton::setSkill(const Skill *skill)
{
    ...
    if (freq == Skill::Frequent
        ...
        if (skill->isChangeSkill()) {
            ...
            _setSkillType(QSanInvokeSkillButton::S_SKILL_CHANGE_FREQUENT);
...
void QSanInvokeSkillDock::update()
{
    if (!_m_buttons.isEmpty()) {
        QList<QSanInvokeSkillButton *> regular_buttons, long_buttons, lordskill_buttons/*, all_buttons*/;
        foreach (QSanInvokeSkillButton *btn, _m_buttons) {
            ...
            else if (Sanguosha->translate(btn->getSkill()->objectName()).size() > 2)
                long_buttons << btn;
            else
                regular_buttons << btn;
        }
        //all_buttons = regular_buttons + lordskill_buttons;
        //all_regular_buttons = long_buttons + regular_buttons;

        int numButtons = regular_buttons.length();
        int numButtonsL = long_buttons.length();
        int lordskillNum = lordskill_buttons.length();
        //Q_ASSERT(lordskillNum <= 6); // HuangTian, ZhiBa and XianSi
        int rows = numButtonsL + qCeil(1.0 * numButtons / 3);
        ...
        int remainingRBtns = numButtons;
        for (int i = 0; i < rows; i++) {
            if (i < numButtonsL) {
                btnNum[i] = 1;
            } else {
                btnNum[i] = qMin(3, remainingRBtns);
                remainingRBtns -= 3;
            }
        }
        if (rows-numButtonsL > 1 && btnNum[rows-1] == 1 && btnNum[rows-2] == 3) {   //...,3,1 -> ...,2,2
            btnNum[rows-1] = 2;
            btnNum[rows-2] = 2;
        }
        //        if (lordskillNum > 3) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0; j < btnNum[i]; j++) {
                if (m < numButtonsL) {
                    QSanInvokeSkillButton *button = long_buttons[m++];
                    button->setButtonWidth((QSanInvokeSkillButton::SkillButtonWidth)(btnNum[i] - 1));
                    button->setPos(btnWidth * j, rowTop);
                } else {
                    QSanInvokeSkillButton *button = regular_buttons[m-numButtonsL];
                    m++;
                    button->setButtonWidth((QSanInvokeSkillButton::SkillButtonWidth)(btnNum[i] - 1));
                    button->setPos(btnWidth * j, rowTop);
                }
