...
void PlayerCardContainer::updatePile(const QString &pile_name)
{
    ...
    QList<int> pile;
    if (pile_name == "OVERT") {
        QList<const Card *> pcards = player->getPublicCards();
        foreach (const Card *card, pcards) {
            if (card->isOvert())
                pile << card->getId();
        }
    } else {
...
void PlayerCardContainer::updateHandcardNum()
{
    ...
    Q_ASSERT(num >= 0);
    QString show_char = QString::number(num);
    if (m_player && m_player->getMark("hide_handcardnum")>0) show_char = "?"; //handcardnum invisible when player has mark "hide_handcardnum"
    _m_layout->m_handCardFont.paintText(_m_handCardNumText, _m_layout->m_handCardArea,
        Qt::AlignCenter, show_char);
