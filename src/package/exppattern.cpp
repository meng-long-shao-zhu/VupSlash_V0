...
// 5th part means whether the card should has any flag or not
// list the flag names, ',' means "or", '~' means "not"
bool ExpPattern::matchOne(const Player *player, const Card *card, QString exp) const
{
    ...
    checkpoint = false;
    QString flag_names = factors.at(4);
    if (!player || flag_names == ".") checkpoint = true;
    if (!checkpoint) {
        QList<int> ids;
        if (card->isVirtualCard())
            ids = card->getSubcards();
        ...
        if (!ids.isEmpty()) {
            foreach (int id, ids) {
                checkpoint = false;
                const Card *card = Sanguosha->getCard(id);
                foreach (const QString &_p, flag_names.split(",")) {
                    QString p = _p;
                    if (!p.startsWith("~") && card->hasFlag(p)) {
                        checkpoint = true;
                    } else if(p.startsWith("~") && !card->hasFlag(p.split("~").at(1))) {
                        checkpoint = true;
                    }
                    if (checkpoint)
                        break;
