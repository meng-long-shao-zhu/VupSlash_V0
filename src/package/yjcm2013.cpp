...
class NewJingce : public TriggerSkill
{
public:
    NewJingce() : TriggerSkill("newjingce")
    {
    ...
    bool trigger(TriggerEvent event, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (event == EventPhaseEnd) {
            ...
            int n = player->tag["newjingce_type"].toList().length();
            if (n <= 0) return false;
            player->tag.remove("newjingce_type");
            ...
            player->tag.remove("newjingce_type");
            room->setPlayerProperty(player, "newjingce_suits", QStringList());
        }else {
            if (player->getPhase() != Player::Play) return false;
            const Card *card = NULL;
            ...
            if (used_card != NULL) {
                QVariantList list = player->tag["newjingce_type"].toList();
                int n = used_card->getTypeId();
                if (!list.contains(QVariant(n))) {
                    list << n;
                    player->tag["newjingce_type"] = list;
                }
            }
            if (card != NULL) {
                QString suit = card->getSuitString();
                QStringList list = player->property("newjingce_suits").toStringList();
                if (!list.contains(suit)) {
                    list << suit;
                    room->setPlayerProperty(player, "newjingce_suits", list);
                    room->addPlayerMark(player, "newjingce_maxcard-Clear");
                }
            }
...
class NewJingceKeep : public MaxCardsSkill
{
public:
    NewJingceKeep() : MaxCardsSkill("#newjingce-keep")
    {
        frequency = Frequent;
    }
    ...
    {
        if (target->hasSkill("newjingce"))
            return target->getMark("newjingce_maxcard-Clear");
        else
            return 0;
    }
};
    ...
    : Package("YJCM2013")
{
    ...
    General *new_guohuai = new General(this, "new_guohuai", "wei", 3);
    new_guohuai->addSkill(new NewJingce);
    new_guohuai->addSkill(new NewJingceKeep);
    related_skills.insertMulti("newjingce", "#newjingce-keep");
