...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *wolong, QVariant &data) const
    {
        ...
        int index = 1;
        if (wolong->getGeneralName().contains("pangtong") || wolong->getGeneral2Name().contains("pangtong"))
            index += qrand() % 2 + 3;
        else if (wolong->isJieGeneral())
            index += qrand() % 2 + 1;
        room->sendCompulsoryTriggerLog(wolong, objectName(), true, true, index);
