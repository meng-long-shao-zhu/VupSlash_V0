...
void WuqianCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (effect.from->getMark("&warth") > 0)
...
public:
    ...
    bool isEnabledAtPlay(const Player *player) const
    {
        return player->getMark("&wrath") >= 2 || player->getMark("&wrath!") >= 2;
    }
...
public:
    ...
    bool onPhaseChange(ServerPlayer *target) const
    {
        if (target->getPhase() != Player::Start || target->getMark(objectName()) > 0 || target->getPile("cychu").length() < 3) return false;
        Room *room = target->getRoom();
        room->broadcastSkillInvoke(objectName());
        ...
        if (room->changeMaxHpForAwakenSkill(target)) {
            ...
            delete dummy;
            QStringList skills;
            if (!target->hasSkill("tenyearjianxiong"))
                skills << "tenyearjianxiong";
            if (!target->hasSkill("tianxing"))
                skills << "tianxing";
    ...
    : Package("god")
{
    ...
    General *shenlvbu3 = new General(this, "shenlvbu3", "god", 4);
