...
AI::Relation AI::GetRelation2v2(const ServerPlayer *a, const ServerPlayer *b)
{
...
void TrustAI::askForGuanxing(const QList<int> &cards, QList<int> &up, QList<int> &bottom, int guanxing_type, int up_limit, int down_limit)
{
    Q_UNUSED(bottom);
    Q_UNUSED(guanxing_type);

    if (guanxing_type == Room::GuanxingDownOnly || up_limit == 0) {
        bottom = cards;
        up.clear();
    } else if (guanxing_type == Room::GuanxingUpOnly || down_limit == 0) {
        up = cards;
        bottom.clear();
    } else {
        /*up.clear();
        ...
        foreach (int id, cards) {
            if (up.length() < up_limit)
                up << id;
            //else if (bottom.length() < down_limit)
            //    bottom << id;
            else
                bottom << id;
        }*/
...
void LuaAI::askForGuanxing(const QList<int> &cards, QList<int> &up, QList<int> &bottom, int guanxing_type, int up_limit, int down_limit)
{
    lua_State *L = room->getLuaState();
    ...
    lua_pushinteger(L, up_limit);
    lua_pushinteger(L, down_limit);

    int error = lua_pcall(L, 5, 2, 0);
    if (error) {
        reportError(L);
        return TrustAI::askForGuanxing(cards, up, bottom, guanxing_type, up_limit, down_limit);
    }
