...
void Room::gameOver(const QString &winner)
{
    ...
    QVariant data = QVariant::fromValue(winner);
    thread->trigger(BeforeGameOver, this, getOwner(), data);
...
void Room::detachSkillFromPlayer(ServerPlayer *player, const QString &skill_name, bool is_equip, bool acquire_only, bool event_and_log, bool stop_huashen)
{
    ...
    if (skill->isLordSkill()) {
        if (!player->hasLordSkill(skill, true)) return;
    } else {
...
void Room::handleAcquireDetachSkills(ServerPlayer *player, const QStringList &skill_names, bool acquire_only, bool getmark, bool event_and_log, bool stop_huashen)
{
    ...
    foreach (QString skill_name, skill_names) {
        if (skill_name.startsWith("-")) {
            ...
            if (!skill) continue;
            if (skill->isLordSkill()) {
                if (!player->hasLordSkill(skill, true)) continue;
            } else {
            ...
            if (!skill) continue;
            if (skill->isLordSkill()) {
                if (player->hasLordSkill(skill_name, true)) continue;
            } else {
...
void Room::acquireTurnEndSkills(ServerPlayer *player,const QString &skill_name, const QStringList &skill_names)
{
    ...
    if (skilllist.contains(skill_name)) {
        ...
        skilllist << skill_name;
        setPlayerProperty(player, "TurnEndSkill", skilllist);
        foreach (QString str, skill_names) {
            if (player->hasSkill(str, true)) continue;
            get << str;
        }
...
void Room::acquireTurnEndSkills(ServerPlayer *player, const QString &skill_name, const QString &skill_names)
{
    acquireTurnEndSkills(player, skill_name, skill_names.split("|"));
}
...
QString Room::askForChoice(ServerPlayer *player, const QString &skill_name, const QString &choices, const QVariant &data)
{
    ...
    QVariant decisionData = QVariant::fromValue("skillChoice:" + skill_name.split("+").at(0) + ":" + answer);
    thread->trigger(ChoiceMade, this, player, decisionData);
    return answer;
}
    ...
    bool handcard_visible, Card::HandlingMethod method, const QList<int> &disabled_ids)
{
    ...
    if (!who->isKongcheng()) {
        if (handcard_visible) {   //pass all handcards to client
            QList<int> handcards = who->handCards();
            JsonArray arg;
            arg << who->objectName();
            arg << JsonUtils::toJsonArray(handcards);
            doNotify(player, S_COMMAND_SET_KNOWN_CARDS, arg);
            //doNotify(player, S_COMMAND_SET_PUBLIC_CARDS, arg);
        } else {
            QList<int> handcard_ids = who->handCards();
            QList<int> overt_handcards;
            foreach(int card_id, handcard_ids)
                if (Sanguosha->getCard(card_id)->isOvert())
                    overt_handcards << card_id;
            if (overt_handcards.length() > 0) {
                JsonArray arg;
                arg << who->objectName();
                arg << JsonUtils::toJsonArray(overt_handcards);
                doNotify(player, S_COMMAND_SET_PUBLIC_CARDS, arg);
            }
        }
    }
    ...
    /*if (who != player && !handcard_visible
        ...
        if (ai) {
            ...
            if (card_id == Card::S_UNKNOWN_CARD_ID) {
                QList<const Card *> cards = who->getCards("h");
                if (method == Card::MethodDiscard) {
                    foreach (const Card *card, cards) {
                        if (!player->canDiscard(who, card->getEffectiveId()) || disabled_ids.contains(card->getEffectiveId()) || card->isOvert())
                            cards.removeOne(card);
                    }
                }
                Q_ASSERT(!cards.isEmpty());
    ...
    bool isSecondaryHero, bool sendLog)
{
    ...
    if (isSecondaryHero) {
        setPlayerProperty(player, "yinni_general", QString());
        changePlayerGeneral2(player, new_general);
    } else {
        setPlayerProperty(player, "yinni_general2", QString());
...
void Room::addRobotCommand(ServerPlayer *player, const QVariant &arg)
{
    ...
    for (int i = 0; i < add_num; i++) {
        ...
        const QString robot_avatar = "zhangege";
...
void Room::assignGeneralsForPlayers(const QList<ServerPlayer *> &to_assign)
{
    ...
    QStringList choices_robot = Sanguosha->getRandomGenerals(total - existed.size(), existed, NULL, true);
    ...
    foreach (ServerPlayer *player, to_assign) {
        ...
        for (int i = 0; i < choice_count; i++) {
            QString choice =  player->getState() == "robot" ? player->findReasonable(choices_robot, true) : player->findReasonable(choices, true);
            if (choice.isEmpty()) break;
            player->addToSelected(choice);
            choices.removeOne(choice);
            choices_robot.removeOne(choice);
        }
    }
}
...
void Room::chooseGenerals(QList<ServerPlayer *> players)
{
    ...
    if (!Config.EnableHegemony && the_lord && players.contains(the_lord)) {
        bool is_robot = the_lord->getState() == "robot";
        QStringList lord_list;
        if (Config.EnableSame)
            lord_list = Sanguosha->getRandomGenerals(Config.value("MaxChoice", 5).toInt(), QSet<QString>(), QString(), is_robot == true);
        /*else if (the_lord->getState() == "robot")
            ...
            lord_list = Sanguosha->getRandomLords(is_robot == true);
...
void Room::recover(ServerPlayer *player, const RecoverStruct &recover, bool set_emotion, int max_recover)
{
    if (player->isDead() || recover.recover <= 0) return;
    if (max_recover == 0) max_recover = player->getMaxHp();
    ...
    if (recover_struct.recover > max_recover - player->getHp()) {
        over_heal = qMin(recover_struct.recover - (max_recover - player->getHp()), recover_struct.recover);
        recover_struct.recover = qMin(max_recover - player->getHp(), recover_struct.recover);
    }
    ...
    if (over_heal > 0) {
        LogMessage log;
        log.from = player;
        log.type = "#OverHealLog";
        log.arg = QString::number(over_heal);
        sendLog(log);

        RecoverStruct over_recover_struct = recover;
        over_recover_struct.recover = over_heal;
        QVariant data = QVariant::fromValue(over_recover_struct);
        thread->trigger(OverHealing, this, player, data);
    }
}
...
void Room::startGame()
{
    ...
    if (mode != "08_defense") {
        ...
        m_players.at(next_list.last())->setNext(m_players.at(next_list.first()));
    }
...
void Room::acquireSkill(ServerPlayer *player, const Skill *skill, bool open, bool getmark, bool event_and_log)
{
    ...
    if (skill->isLordSkill()) {
        if (player->hasLordSkill(skill_name, true)) return;
    } else {
...
QList<int> Room::askForGuanxing(ServerPlayer *zhuge, const QList<int> &cards, GuanxingType guanxing_type, bool sendLod, int up_limit, int down_limit)
{
    ...
    if(up_limit + down_limit < cards.length())
        down_limit = cards.length() - up_limit;

    AI *ai = zhuge->getAI();
    if (ai) {
        ai->askForGuanxing(cards, top_cards, bottom_cards, (int)guanxing_type, up_limit, down_limit);
        ...
        guanxingArgs << up_limit;
        guanxingArgs << down_limit;
    ...
    bool result_legal = (top_cards.length() <= up_limit) && (bottom_cards.length() <= down_limit);
    if (!length_equal || !result_equal || !result_legal) {
        if (guanxing_type == GuanxingDownOnly || up_limit == 0) {
            bottom_cards = cards;
            top_cards.clear();
        } else if (guanxing_type == GuanxingUpOnly || down_limit == 0) {
            top_cards = cards;
            bottom_cards.clear();
        } else {
            top_cards.clear();
...
void Room::setOvertCard(ServerPlayer *player, int card_id, bool can)
{
    if (getCardOwner(card_id) != player) return;
    ...
    QList<int> handcard_ids = player->handCards();
    QList<int> overt_handcards;
    foreach(int card_id, handcard_ids)
        if (Sanguosha->getCard(card_id)->isOvert())
            overt_handcards << card_id;
    if (overt_handcards.length() > 0) {
        ...
        arg << player->objectName();
        arg << JsonUtils::toJsonArray(overt_handcards);
        doBroadcastNotify(S_COMMAND_SET_PUBLIC_CARDS, arg);
    }
    ...
    if (can) {
        ...
        card_ids << card_id;
        QVariant data = QVariant::fromValue(card_ids);
        thread->trigger(CardOvert, this, player, data);
    }

    //doNotify(player, S_COMMAND_REFRESH_OVERT_PILE, QVariant(player->objectName()));
    //doBroadcastNotify(S_COMMAND_REFRESH_OVERT_PILE, QVariant(player->objectName()));
}
...
QString Room::askForChooseCardName(ServerPlayer *player, const QString &type, bool refusable, const QString &reason)
{
    QStringList type_list = type.split("+");
    QStringList alllist;
    QList<int> ids;
    QList<int> from_ids;
    if (type_list[0] == "a") {
        from_ids = Sanguosha->getRandomCards();
    } else {
        if (type_list[0].contains("m")) {
            from_ids += getDrawPile();
        }
        if (type_list[0].contains("q")) {
            from_ids += getDiscardPile();
        }
    }
    if (from_ids.isEmpty()) return "";
    qSort(from_ids.begin(), from_ids.end());
    foreach(int id, from_ids) {
        const Card *c = Sanguosha->getEngineCard(id);
        if (!type_list[1].contains("b") && c->isKindOf("BasicCard")) continue;
    ...
    if (ids.isEmpty()) return "";
    fillAG(ids, player, QList<int>(), true);
    int id = -1;
    id = askForAG(player, ids, refusable, reason);
    clearAG(player);
    if (id > -1) {
        return Sanguosha->getEngineCard(id)->objectName();
    } else {
        return "";
    }
}
